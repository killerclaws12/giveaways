  ...
  run: async (client, interaction) => {
    ...
    if (!interaction.member.permissions.has('MANAGE_MESSAGES') && !interaction.member.roles.cache.some((r) => r.name === "Giveaways")) {
      return interaction.reply({
        content: '<:error:996259473465294930> | You need to have the manage messages permissions to start giveaways.',
        ephemeral: true
      });
    }
    ...
    if (!giveawayChannel.isText()) {
      return interaction.reply({
        content: '<:error:996259473465294930> | Please select a text channel!',
        ephemeral: true
      });
    }
   if(isNaN(ms(giveawayDuration))) {
    return interaction.reply({
      content: '<:error:996259473465294930> | Please select a valid duration!',
      ephemeral: true
    });
  }
    if (giveawayWinnerCount < 1) {
      return interaction.reply({
        content: '<:error:996259473465294930> | Please select a valid winner count! greater or equal to one.',
      })
    }
    ...
    if (bonusRole) {
      if (!bonusEntries) {
        return interaction.reply({
          content: `<:error:996259473465294930> | You must specify how many bonus entries would ${bonusRole} recieve!`,
          ephemeral: true
        });
      }
    }
